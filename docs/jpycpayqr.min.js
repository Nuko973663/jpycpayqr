...
const txt_version="2021.08.16.0";var default_dest_addr="0xafd382aCC893127D6fbb197b87453070Fc14D43d";const jpyc_contract_addr="0x6ae7dfc73e0dde2aa99ac063dcf7e8a63265108c",polygon_chain_id="137",list=["1","2","3","4","5","6","7","8","9","0"];$((function(){let id_pref="#btn-";list.forEach(e=>{$("#btn-"+e).on("click",()=>{$("#amount").val($("#amount").val()+e)})}),$("#btn-dot").on("click",()=>{-1==$("#amount").val().indexOf(".")&&$("#amount").val($("#amount").val()+".")}),$("#btn-clear").on("click",()=>{$("#amount").val("")}),$("#btn-generate").on("click",()=>{$("#amount").val.length>0&&(makeCode(),move_to_link($("#qrcode")))}),$("#config").on("click",()=>{let dest=window.prompt("Input Destination Addr","");null!=dest&&(set_dest_addr(dest),get_dest_addr())}),$("#openMetamask").on("click",()=>{window.location.href="https://metamask.app.link/dapp/nuko973663.github.io/jpycpayqr/"}),$("#version").text(txt_version);var qrcode=new QRCode(document.getElementById("qrcode"),{width:300,height:300});function makeCode(){let amount=$("#amount").val(),dest_addr=get_dest_addr(),code="ethereum:pay-"+jpyc_contract_addr+"@137/transfer?address="+dest_addr+"&uint256="+amount+"e18";qrcode.makeCode(code)}function get_dest_addr(){let dest_addr=localStorage.getItem("dest_addr");return null===dest_addr&&(dest_addr=default_dest_addr),$("#dest").text(dest_addr),dest_addr}function set_dest_addr(addr){localStorage.setItem("dest_addr",addr)}function move_to_link(link){var position=link.offset().top;$("body,html").animate({scrollTop:position},400,"swing")}window.addEventListener("load",async()=>{let accounts=[];if(window.ethereum){window.web3=new Web3(ethereum);try{await ethereum.enable(),accounts=await web3.eth.getAccounts()}catch(error){$("#askMetamask").modal("show")}}else if(window.web3)try{window.web3=new Web3(web3.currentProvider),accounts=await web3.eth.getAccounts()}catch(error){$("#askMetamask").modal("show")}else $("#askMetamask").modal("show");accounts.length>0?set_dest_addr(accounts[0]):$("#askMetamask").modal("show"),get_dest_addr()})}));